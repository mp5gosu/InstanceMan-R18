...
public:
	...
	Bool Execute(BaseDocument* doc) override
	{
		...
		const auto swapTargetClone = static_cast<BaseObject*>(swapTarget->GetClone(COPYFLAGS_NO_HIERARCHY, nullptr));
		...
		if (bCtrl)
		{
			...
			for (auto i = 0; i < refObjChildren->GetCount(); ++i)
			{
				...
				doc->AddUndo(UNDOTYPE_DELETE, child);
				child->Remove();
		...
		doc->AddUndo(UNDOTYPE_CHANGE, swapTarget);
