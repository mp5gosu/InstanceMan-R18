...
inline Bool g_LinkInstance(BaseObject* instance, BaseObject* refObj)
{
	...
	doc->AddUndo(UNDOTYPE_CHANGE_SMALL, instance);
	instance->SetParameter(DescID(INSTANCEOBJECT_LINK), data, DESCFLAGS_SET_0);
...
inline BaseObject* g_CreateInstance(BaseObject* refObj)
{
	...
	doc->AddUndo(UNDOTYPE_NEW, instanceObject);

	// Get the active instance mode from plugin settings
	auto instanceMode = false;
	const auto bc = GetWorldPluginData(PID_IM);
	if (bc)
		instanceMode = bc->GetBool(DEFAULT_INSTANCEMODE, false);


	// Set Name to the same as the reference object
	doc->AddUndo(UNDOTYPE_CHANGE_SMALL, instanceObject);
	instanceObject->SetName(refObj->GetName());
	doc->AddUndo(UNDOTYPE_CHANGE_SMALL, instanceObject);
	instanceObject->SetParameter(INSTANCEOBJECT_RENDERINSTANCE, GeData(instanceMode), DESCFLAGS_SET_0);

	g_LinkInstance(instanceObject, refObj);


	// Deselect reference object and select new instance object
	doc->AddUndo(UNDOTYPE_BITS, refObj);
	refObj->DelBit(BIT_ACTIVE);
	doc->AddUndo(UNDOTYPE_BITS, instanceObject);
...
inline void g_MoveChildren(BaseObject* srcObj, BaseObject* tgtObj)
{
	...
	while (child)
	{
		...
		doc->AddUndo(UNDOTYPE_CHANGE, child);
		child->Remove();
...
inline void g_CreateInstanceCopy(BaseObject* obj)
{
	...
	const auto copyObj = static_cast<BaseObject*>(obj->GetClone(COPYFLAGS_RECURSIONCHECK, nullptr));
	if (!copyObj)
		return;
	...
	doc->AddUndo(UNDOTYPE_NEW, copyObj);
...
inline BaseObject* g_GetInstanceRef(BaseObject* obj, const Bool deep = false)
{
	...
	GeData data;
	if (!obj->GetParameter(DescID(INSTANCEOBJECT_LINK), data, DESCFLAGS_GET_0))
		return nullptr;
	...
	while (linkedEntity && deep && linkedEntity->IsInstanceOf(Oinstance))
	{
		if (refCountLimit <= 0)
		{
			MessageDialog(resource.LoadString(IDS_REFCOUNTWARN));
			return nullptr;
		}

		if (!linkedEntity->GetParameter(DescID(INSTANCEOBJECT_LINK), data, DESCFLAGS_GET_0))
			return nullptr;
...
inline BaseObject* g_MakeInstanceEditable(BaseObject* obj, const bool deep = false)
{
	...
	auto refObj = obj;
	doc->AddUndo(UNDOTYPE_CHANGE, refObj);
	if (obj->IsInstanceOf(Oinstance))
	{
		...
		return static_cast<BaseObject*>(refObj->GetClone(COPYFLAGS_0, nullptr));
	}
	return nullptr;
}
